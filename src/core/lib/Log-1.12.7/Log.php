...
class Log {
    ...
    * @access protected
    */
   var $_formatMap = array('%{timestamp}' => '%1$s', '%{ident}' => '%2$s', '%{priority}' => '%3$s', '%{message}' => '%4$s', '%{file}' => '%5$s', '%{line}' => '%6$s', '%{function}' => '%7$s', '%{class}' => '%8$s', '%\{' => '%%{');

   /**
    * Attempts to return a concrete Log instance of type $handler.
    *
    * @param string $handler
    *           The type of concrete Log subclass to return.
    *           Attempt to dynamically include the code for
    *           this subclass. Currently, valid values are
    *           'console', 'syslog', 'sql', 'file', and 'mcal'.
    *           
    * @param string $name
    *           The name of the actually log file, table, or
    ...
    * @since Log 1.0
    */
   public static function factory($handler, $name = '', $ident = '', $conf = array(), $level = PEAR_LOG_DEBUG) {
      $handler = strtolower($handler);
      $class = 'Log_' . $handler;
      $classfile = 'Log/' . $handler . '.php';
      
      /*
       * Attempt to include our version of the named class, but don't treat a failure as fatal. The caller may have already included their own version of the named class.
       */
    ...
    * @since Log 1.0
    */
   public static function singleton($handler, $name = '', $ident = '', $conf = array(), $level = PEAR_LOG_DEBUG) {
      static $instances;
      if (! isset($instances))
         $instances = array();
    ...
    *
    * @param mixed $message
    *           String or object containing the message
    *           to log.
    *           
    ...
    *
    * @param mixed $message
    *           The original message data. This may be a
   ...
   function _extractMessage($message) {
      /*
       * If we've been given an object, attempt to extract the message using a known method. If we can't find such a method, default to the "human-readable" version of the object. We also use the human-readable format for arrays.
       */
    ...
    *
    * @param int $depth
    *           The initial number of frames we should step
    *           back into the trace.
    *           
    ...
    * the function that invoked the log system.
    * This is used on conjunction
    * with the 'file', 'line', 'function', and 'class' formatters.
    *
    * @param int $depth
    *           The new backtrace depth.
    *           
    * @access public
    * @since Log 1.12.7
    */
   ...
   function _format($format, $timestamp, $priority, $message) {
      /*
       * If the format string references any of the backtrace-driven variables (%5 %6,%7,%8), generate the backtrace and fetch them.
       */
      ...
      /*
       * Build the formatted string. We use the sprintf() function's "argument swapping" capability to dynamically select and position the variables which will ultimately appear in the log string.
       */
      return sprintf($format, $timestamp, $this->_ident, $this->priorityToString($priority), $message, isset($file) ? $file : '', isset($line) ? $line : '', isset($func) ? $func : '', isset($class) ? $class : '');
   }
    ...
    * representation of a priority name.
    ...
    * given priority.
    * In other words, $priority will be the lowest priority
    ...
    * given priority.
    * In other words, $priority will be the highests priority
    * matched by the resulting mask.
    *
    * This method may be called statically.
    *
    ...
    *           The Log_observer instance to attach as a
    *           listener.
    *           
    * @param
    ...
    *
    * @param object $observer
    *           The Log_observer instance to detach from

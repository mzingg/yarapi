...
class Log_mail extends Log {
      ...
      if ($this->_opened) {
         if ($this->_shouldSend && ! empty($this->_message)) {
            if ($this->_mailBackend === '') { // use mail()
               ...
               $headers = array('From' => $this->_from, 'To' => $this->_recipients, 'User-Agent' => 'PEAR Log Package', 'Subject' => $this->_subject);
               $mailer = &Mail::factory($this->_mailBackend, $this->_mailParams);
               $res = $mailer->send($this->_recipients, $headers, $this->_message);
               if (PEAR::isError($res)) {
                  return false;
               }
            }
   ...
   function flush() {
      /*
       * It's sufficient to simply call close() to flush the output. The next call to log() will cause the handler to be reopened.
       */
      return $this->close();
   }
    ...
    *           The priority of the message. Valid

...
EOT;
   ...
   function close() {
      /*
       * If there are still lines waiting to be written, open the output window so that we can drain the buffer.
       */
      if (! $this->_opened && (count($this->_buffer) > 0)) {
         $this->open();
      }
    ...
    * The message is also passed along

...
class Log_composite extends Log {
    ...
    * It is assumed that all of the children
   ...
   function log($message, $priority = null) {
      ...
      /*
       * Abort early if the priority is above the composite handler's maximum logging level. XXX: Consider whether or not introducing this change would break backwards compatibility. Some users may be expecting composite handlers to pass on all events to their children regardless of their own priority.
       */
      // f (!$this->_isMasked($priority)) {
      // return false;
      //
      

      /*
       * Iterate over all of our children. If a unopened child will respond to this log event, we attempt to open it immediately. The composite handler's opened state will be enabled as soon as the first child handler is successfully opened. We track an overall success state that indicates whether or not all of the relevant child handlers were opened and successfully logged the event. If one handler fails, we still attempt any remaining children, but we consider the overall result a failure.
       */
      $success = true;
      foreach ($this->_children as $child) {
         ...
         if (! $child->_opened) {
            ...
            /*
             * If we've successfully opened our first handler, the composite handler itself is considered to be opened.
             */

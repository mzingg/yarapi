...
class Module {
   ...
   public function __construct($sModuleName, $sAbsoluteDirectoryPath) {
      if (! is_dir($sAbsoluteDirectoryPath) || ! is_readable($sAbsoluteDirectoryPath)) {
         yarapi_log(sprintf('Module directory (%s) for module %s does not exist or is not readable.', $sAbsoluteDirectoryPath, $sModuleName), PEAR_LOG_ERR);
      }
   ...
   public function load() {
      ...
      if ($this->getStatus() != 'enabled' || $this->isLoaded())
         return;
      
      $sModulePhpFile = $this->getModulePhpFileName();
      if (! file_exists($sModulePhpFile) || ! is_readable($sModulePhpFile)) {
         yarapi_log(sprintf('Module entry file (%s) for module %s does not exist or is not readable.', $sModulePhpFile, $this->getName()), PEAR_LOG_WARNING);
         return;
      }
      ...
      eval(sprintf('function %s() { return Modules::findModuleByName("%s"); }', $this->getLoadIndicatorFunctionName(), $this->getName()));
   ...
   public function providesHook($sHookName) {
      ...
      if (! $bModuleIsLoaded && $bFunctionExists) {
         yarapi_log(sprintf('Hook %s (%s) was defined by a wrong module (NOT %s). Please check your modules for wrong hook definitions.', $sHookName, $sFunctionName, $this->getName()), PEAR_LOG_ERR);
         return false;
      }
   ...
   public function callHook() {
      ...
      if (! $this->providesHook($sHookName))
         return null;
   ...
   public function getPath() {
      return $this->sAbsoluteDirectoryPath;
   }
   ...
   public function getCompatibleMajor() {
      return $this->nCompatibleMajor;
   }

   public function getCompatibleMinor() {
      return $this->nCompatibleMinor;
   }

   public function getCompatibleRevision() {
      return $this->sCompatibleRevision;
   }
   ...
   public function getVersionRevision() {
      return $this->sVersionRevision;
   }

   private function getHookFunctionName($sHookName) {
      return $this->getName() . '_' . $sHookName;
   }

   private function getLoadIndicatorFunctionName() {
      return '__' . $this->getName() . '_module';
   }

   private function getModulePhpFileName() {
      return $this->getPath() . '/' . $this->getName() . self::$MODULE_SUFFIX;
   }
   ...
   private function applyInfoData() {
      ...
      if (! array_key_exists('enabled', $aModuleInfo)) {
         yarapi_log(sprintf('Module info for module [%s] does not contain the required key "enabled".', $this->sModuleName), PEAR_LOG_ERR);
         return;
      }
      ...
      if (array_key_exists('compatible', $aModuleInfo)) {
         ...
         if (count($aVersionParts) == 3)
            list($nYarapiMajor, $nYarapiMinor, $nYarapiRevision) = $aVersionParts;
         else
            list($nYarapiMajor, $nYarapiMinor) = $aVersionParts;
            
            // now compatible value
         $aVersionParts = explode('.', $aModuleInfo['compatible'], 3);
         if (count($aVersionParts) == 3)
            list($this->nCompatibleMajor, $this->nCompatibleMinor, $this->sCompatibleRevision) = $aVersionParts;
         else
            list($this->nCompatibleMajor, $this->nCompatibleMinor) = $aVersionParts;
      ...
      if (array_key_exists('compatible', $aModuleInfo)) {
         $aVersionParts = explode('.', $aModuleInfo['compatible'], 3);
         if (count($aVersionParts) == 3)
            list($this->nVersionMajor, $this->nVersionMinor, $this->sVersionRevision) = $aVersionParts;
         else
            list($this->nVersionMajor, $this->nVersionMinor) = $aVersionParts;
      }
   ...
   private function readInfoFile() {
      ...
      if (! file_exists($sInfoFilePath) || ! is_readable($sInfoFilePath)) {
         yarapi_log(sprintf('Module info file (%s) for module [%s] does not exist or is not readable.', $sInfoFilePath, $this->sModuleName), PEAR_LOG_WARNING);
         return $aResult;
      }

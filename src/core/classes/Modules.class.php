...
class Modules extends PersistentState {
   ...
   private function scanForModules() {
      ...
      foreach ($aFiles as $sSubEntry) {
         ...
         if (array_key_exists('ignored_directories', $aModulesConfiguration) && in_array($sSubEntry, $aModulesConfiguration['ignored_directories']))
            continue;
         
         $sAbsolutePath = $sModulesDir . '/' . $sSubEntry;
         if (! is_dir($sAbsolutePath))
            continue;
   ...
   private function register(Module $oModule) {
      ...
      if (! is_array($this->aModulesByName))
         $this->aModulesByName = array();
      if (! is_array($this->aModulesByStatus))
         $this->aModulesByStatus = array();
   ...
   protected function getStateData() {
      ...
      $aData = array('modulesByName' => $aSerializedModulesByName, 'modulesByStatus' => $aSerializedModulesByStatus);
   ...
   private function _findModuleByName($sModuleName) {
      if (! array_key_exists($sModuleName, $this->aModulesByName))
         return null;
   ...
   private function _findModulesByStatus($sStatus) {
      ...
      if (! array_key_exists($sStatus, $this->aModulesByStatus))
         return array();

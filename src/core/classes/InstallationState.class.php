...
class InstallationState extends PersistentState {
   ...
   * </ul>
   * 
   * @param string $absoluteInstallationDir Absolute path to the YARAPI installation
   * @param string $softwareVersion YARAPI version string
   * @param string $environment Current environment YARAPI operates in (dev, test, prod, ...)
   * @return void
   */
   ...
   * @param string $directoryPath Absolute path to the directory to check
   * @param boolean $writable If true the function also checks if that directory is writeable. It checks only for readability otherwise.
   * @return void
   */
  private function checkDirectory($directoryPath, $writable = false) {
    // First check for valid parameters
    if (!$directoryPath) throw Exception('A valid directory path must be provided.');
  ...
  private function initialize() {
    ...
    foreach ($aDefaultIncludePaths as $sIncludePath) {
  ...
  public function registerIncludePath($includePath, $absolute = false, $oneTime = false) {
    ...
    if (!file_exists($sPath) || !is_dir($sPath) || !is_readable($sPath)) return;
    
    // Do nothing if the path was already registerd
    if (in_array($sPath, $this->aIncludePath)) return;
    ...
    set_include_path(
      get_include_path() . PATH_SEPARATOR . // original include path; ., pear, ...
      $sPath // and the newly added entry
    );
  }
  ...
  protected function getStateData() {
    $aData = array(
      'installedVersion' => $this->sInstalledVersion,
      'includePath' => $this->aIncludePath
    );

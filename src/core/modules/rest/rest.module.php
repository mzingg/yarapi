...
function rest_request($oTask, $oResult) {
   ...
   if (! $sRequestType || ! in_array($sRequestType, array('artifact', 'relation')))
      return;
   ...
   switch ($sRequestType) {
      case 'artifact' :
         ...
         foreach ($aArtifactsToModule[$sRequestedArtifact] as $sModuleName => $oModule) {
            ...
            if (array_key_exists($sModuleName, $aAllowedCallResult) && $aAllowedCallResult[$sModuleName] === false)
               continue;
            ...
            if (! in_array($sRequestMethod, $aOptions))
               return;

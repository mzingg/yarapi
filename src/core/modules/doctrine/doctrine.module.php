...
/*
 * function doctrine_manager() { static $oManager = null; if (!$oManager) { $oManager = Doctrine_Manager::getInstance(); $oManager->setAttribute(Doctrine::ATTR_AUTOLOAD_TABLE_CLASSES, true); $aConnectionConfig = yarapi_load_config('doctrine_connections'); foreach ($aConnectionConfig as $sConnectionName => $sDsn) { Doctrine_Manager::connection($sDsn, $sConnectionName); } } return $oManager; }
 */
...
function doctrine_command($sCommand) {
   if (! in_array($sCommand, array('createTablesFromModels', 'loadFixtures')))
      return null;
